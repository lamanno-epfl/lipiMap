...
# Code: https://github.com/theislab/scarches/tree/master/scarches/plotting/_alluvial.py
...
class AlluvialTool:
    def __init__(
        ...
        **kwargs,
        ...
        self.res = (
            res  # defines the resolution of the splines for all veins
        )
        self.combs = sorted(
            itertools.product((0, 1), (1, 0)), key=lambda xy: all(xy)
        )
        ...
        self.item_text_len, self.width_text_len = (
            self.get_vein_label_lengths()
        )
    ...
    def get_group_widths(self):
        return [
            self.get_group_width(group)
            for group in (self.a_members, self.b_members)
        ]
    ...
    def generate_alluvial_vein(self, a_item, b_item):
        ...
        a_item_coord = self.item_coord_dic[
            a_item
        ].read_state_and_advance_y(width)
        b_item_coord = self.item_coord_dic[
            b_item
        ].read_state_and_advance_y(width)
        y_range = (
            a_item_coord[1],
    ...
    def get_color_array(
        ...
        cmap = (
            cmap
            if cmap is not None
            else matplotlib.cm.get_cmap("hsv", lci * 10**3)
        )
        color_array = (
            colors
    ...
    def get_vein_label_lengths(self):
        ...
        width_text_len = max(
            [len(str(w)) for w in self.item_widths_dic.values()]
        )
            ...
            for side, sign in enumerate((-1, 1)):
                plt.text(
                    ...
                    * (
                        label_shift
                        + itl
                        + int(disp_width) * (len(wdisp_sep) + wtl)
                    )
                    * self.h_gap_frac,
                    y,
                    labels[side],
