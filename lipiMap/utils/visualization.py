...
def visualize_errors(loss_dict, section=15):
    ...
    for i, ax in enumerate(axs):
        ax.semilogy(
            loss_dict[f"epoch_{loss_types[i]}"],
            label=f"train_{loss_types[i]}",
        )
        ax.semilogy(
            loss_dict[f"val_{loss_types[i]}"], label=f"val_{loss_types[i]}"
        )
        ax.legend()
...
def visualize_single_residuals(adata_orig, adata_recon, lipid):
    ...
    sc0 = axes[0].scatter(
        zz, yy, c=orig, s=dot_size, alpha=1, cmap="viridis", norm=norm
    )
    axes[0].set_title("Original", size=15)
    sc1 = axes[1].scatter(
        zz, yy, c=recon, s=dot_size, alpha=1, cmap="viridis", norm=norm
    )
...
def visualize_program_residuals(
    adata_orig, adata_recon, mask, name, global_ranking
):
    zz = adata_orig.obs["z_index"]
    yy = -adata_orig.obs["y_index"]
    ...
    for i in range(10):
        ...
        sc0 = axes[i][0].scatter(
            zz, yy, c=orig, s=dot_size, alpha=1, cmap="viridis", norm=norm
        )
        axes[i][0].set_title(
            f"Original {lipids_names[local_ranking[i]]}", size=15
        )
        sc1 = axes[i][1].scatter(
            zz, yy, c=recon, s=dot_size, alpha=1, cmap="viridis", norm=norm
        )
        axes[i][1].set_title(
            f"Reconstructed {lipids_names[local_ranking[i]]}", size=15
        )
        ...
        axes[i][2].set_title(
            f"Relative Error {lipids_names[local_ranking[i]]}", size=15
        )
...
def visualize_reconstruction_evolution(
    adata_orig, adata_recon_list, lipid, path=None
):
    zz = adata_orig.obs["z_index"]
    yy = -adata_orig.obs["y_index"]
...
def plot_decoder_weights(model, adata, mask_framework, alpha_l1=None):
    decoder_weights = (
        model.model.decoder.L0.expr_L.weight.data.cpu().numpy()
    )
    ...
    vmin = np.abs(np.nanmin(decoder_weights))
    vmax = np.abs(np.nanmax(decoder_weights))
...
def pdf_spatial_representation(
    adata, what_data, output_file, mask_framework="all"
):
    assert (
        what_data == "X_latent"
        or what_data == "X_reconstruction"
        or what_data == "X_residuals"
    ), "what_data must be either 'X_latent' or 'X_reconstruction' or 'X_residuals'"

    coords = adata.obs[
        ["Section", "x", "y"]
    ]  # 'zccf', 'yccf', 'z_index', 'y_index',

    pdf_pages = PdfPages(output_file)

    elements = (
        adata.uns[mask_framework]
        if what_data == "X_latent"
    ...
    for i, curr in tqdm(enumerate(elements)):
        ...
        fig, axes = plt.subplots(
            num_rows, num_cols, figsize=(20, 3 * num_rows), rasterized=True
        )
