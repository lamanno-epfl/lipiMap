...
# TODO: do the same for surgery in lipimap
...
# Code: https://github.com/theislab/scarches/tree/master/scarches/surgery/trvae.py
...
def trvae_operate(
    ...
    to_be_added_encoder_input_weights = np.random.randn(
        encoder_input_weights.size()[0], n_new_conditions
    ) * np.sqrt(
        2
        / (
            encoder_input_weights.size()[0]
            + 1
            + encoder_input_weights.size()[1]
        )
    )
    to_be_added_encoder_input_weights = (
        torch.from_numpy(to_be_added_encoder_input_weights)
        .float()
        .to(network.device)
    )
    network.encoder.FC.L0.cond_L.weight.data = torch.cat(
        (encoder_input_weights, to_be_added_encoder_input_weights), 1
    )

    decoder_input_weights = network.decoder.FirstL.L0.cond_L.weight
    to_be_added_decoder_input_weights = np.random.randn(
        decoder_input_weights.size()[0], n_new_conditions
    ) * np.sqrt(
        2
        / (
            decoder_input_weights.size()[0]
            + 1
            + decoder_input_weights.size()[1]
        )
    )
    to_be_added_decoder_input_weights = (
        torch.from_numpy(to_be_added_decoder_input_weights)
        .float()
        .to(network.device)
    )
    network.decoder.FirstL.L0.cond_L.weight.data = torch.cat(
        (decoder_input_weights, to_be_added_decoder_input_weights), 1
    )
    ...
    new_trainer.train(n_epochs=n_epochs, lr=0.001)
