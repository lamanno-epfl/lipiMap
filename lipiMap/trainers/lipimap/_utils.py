...
# Code: https://github.com/theislab/scarches/tree/master/scarches/trainers/trvae/_utils.py
...
def train_test_split(
    adata, train_frac=0.85, condition_key=None, labeled_array=None
):
    ...
    if condition_key is not None and condition_key != "arbitrary":
        train_idx = []
        val_idx = []
        ...
        print("conditioning-balanced train/test split")
        ...
        if condition_key == "arbitrary":
            print("arbitrary train/test split, unbalanced wrt conditions")
        else:
            print("unconditioned train/test split")
...
def make_dataset(
    ...
    train_idx, val_idx = train_test_split(
        adata, train_frac, condition_key=condition_key
    )

    print("Instantiating dataset")
    data_set_train = LBADataset(
...
def custom_collate(batch):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        output = {
            key: custom_collate([d[key] for d in batch]) for key in elem
        }
        return output
