...
# Code: https://github.com/theislab/scarches/tree/master/scarches/trainers/expimap/regularized.py
...
class ProxGroupLasso:
    ...
    def __call__(self, W):
        ...
        norm_g_gr_vect = (
            norm_vect > self._group_coeff
        )  # --> 1 if ||W_:,j||_2 > group_coeff (alpha*eta in the paper)
        ...
        W *= (
            norm_g_gr_vect.float()
        )  # --> W = W * 1 if ||W_:,j||_2 > group_coeff
...
class lipiMapTrainer(Trainer):
    ...
    alpha_l1: Float
        ...
        Specifies the strength for deactivating the lipids which are not in the corresponding annotations
        groups in the mask.
    ...
    def check_prox_ops(self):
        ...
        use_prox_ops["main_group_lasso"] = (
            use_main and self.alpha_gl is not None
        )

        use_mask = use_main and self.model.mask is not None
        use_prox_ops["main_soft_mask"] = (
            use_mask and self.alpha_l1 is not None
        )

        use_ext = (
            self.model.n_ext_decoder > 0 and self.gamma_ext is not None
        )
        use_ext = (
            use_ext and self.model.decoder.L0.ext_L.weight.requires_grad
        )
        use_prox_ops["ext_unannot_l1"] = use_ext

        use_ext_m = (
            self.model.n_ext_m_decoder > 0 and self.alpha_l1 is not None
        )
        use_ext_m = (
            use_ext_m
            and self.model.decoder.L0.ext_L_m.weight.requires_grad
        )
        use_prox_ops["ext_soft_mask"] = (
            use_ext_m and self.model.ext_mask is not None
        )
    ...
    def init_anneal(self):
        ...
        use_soft_mask = (
            self.use_prox_ops["main_soft_mask"]
            or self.use_prox_ops["ext_soft_mask"]
        )
        if use_soft_mask and self.alpha_l1_epoch_anneal is not None:
            corr_coeffs["alpha_l1"] = 1.0 / self.alpha_l1_epoch_anneal
        else:
            corr_coeffs["alpha_l1"] = 1.0

        if (
            self.use_prox_ops["ext_unannot_l1"]
            and self.gamma_epoch_anneal is not None
        ):
            corr_coeffs["gamma_ext"] = 1.0 / self.gamma_epoch_anneal
        else:
            corr_coeffs["gamma_ext"] = 1.0
    ...
    def anneal(self):
        ...
        if self.corr_coeffs["gamma_ext"] < 1.0:
            any_change = True
            time_to_anneal = (
                self.epoch > 0 and self.epoch % self.gamma_anneal_each == 0
            )
            if time_to_anneal:
                self.corr_coeffs["gamma_ext"] = min(
                    self.epoch / self.gamma_epoch_anneal, 1.0
                )
                if self.print_stats:
                    print(
                        "New gamma_ext anneal coefficient:",
                        self.corr_coeffs["gamma_ext"],
                    )

        if self.corr_coeffs["alpha_l1"] < 1.0:
            any_change = True
            time_to_anneal = (
                self.epoch > 0
                and self.epoch % self.alpha_l1_anneal_each == 0
            )
            if time_to_anneal:
                self.corr_coeffs["alpha_l1"] = min(
                    self.epoch / self.alpha_l1_epoch_anneal, 1.0
                )
                if self.print_stats:
                    print(
                        "New alpha_l1 anneal coefficient:",
                        self.corr_coeffs["alpha_l1"],
                    )
    ...
    def init_prox_ops(self):
        ...
        if (
            "main_group_lasso" not in self.prox_ops
            and self.use_prox_ops["main_group_lasso"]
        ):
            print(
                "Init the group lasso proximal operator for the main terms."
            )
            alpha_gl_corr = self.alpha_gl * self.watch_lr
            self.prox_ops["main_group_lasso"] = ProxGroupLasso(
                alpha_gl_corr, self.omega
            )

        if (
            "main_soft_mask" not in self.prox_ops
            and self.use_prox_ops["main_soft_mask"]
        ):
            print(
            ...
            self.prox_ops["main_soft_mask"] = ProxL1(
                alpha_l1_corr, main_mask
            )

        if (
            "ext_unannot_l1" not in self.prox_ops
            and self.use_prox_ops["ext_unannot_l1"]
        ):
            print(
        ...
        if (
            "ext_soft_mask" not in self.prox_ops
            and self.use_prox_ops["ext_soft_mask"]
        ):
            print(
            ...
            self.prox_ops["ext_soft_mask"] = ProxL1(
                alpha_l1_corr, ext_mask
            )
    ...
    def apply_prox_ops(self):
        if "main_soft_mask" in self.prox_ops:
            self.prox_ops["main_soft_mask"](
                self.model.decoder.L0.expr_L.weight.data
            )
        if "main_group_lasso" in self.prox_ops:
            self.prox_ops["main_group_lasso"](
                self.model.decoder.L0.expr_L.weight.data
            )
        if "ext_unannot_l1" in self.prox_ops:
            self.prox_ops["ext_unannot_l1"](
                self.model.decoder.L0.ext_L.weight.data
            )
        if "ext_soft_mask" in self.prox_ops:
            self.prox_ops["ext_soft_mask"](
                self.model.decoder.L0.ext_L_m.weight.data
            )
    ...
    def on_epoch_end(self, k):
        if self.print_stats:
            ...
            if self.use_prox_ops["main_soft_mask"]:
                ...
                share_deact_lipids = (
                    self.model.decoder.L0.expr_L.weight.data.abs() == 0
                ) & main_mask
                share_deact_lipids = (
                    share_deact_lipids.float().sum().cpu().numpy()
                    / self.model.n_inact_lipids
                )
                print(
                    "Share of deactivated inactive lipids: %.4f"
                    % share_deact_lipids
                )
                print("-------------------")
            if self.use_prox_ops["ext_soft_mask"]:
                ext_mask = self.prox_ops["ext_soft_mask"]._I
                share_deact_ext_lipids = (
                    self.model.decoder.L0.ext_L_m.weight.data.abs() == 0
                ) & ext_mask
                share_deact_ext_lipids = (
                    share_deact_ext_lipids.float().sum().cpu().numpy()
                    / self.model.n_inact_ext_lipids
                )
                print(
                    "Share of deactivated inactive lipids in extension terms: %.4f"
                    % share_deact_ext_lipids
                )
                print("-------------------")
            if self.use_prox_ops["ext_unannot_l1"]:
                active_lipids = (
                    self.model.decoder.L0.ext_L.weight.data.abs()
                    .cpu()
                    .numpy()
                    > 0
                ).sum(0)
                print(
                    "Active lipids in unannotated extension terms:",
                    active_lipids,
                )
                sparse_share = 1.0 - active_lipids / self.model.input_dim
                print(
                    "Sparcity share in unannotated extension terms:",
                    sparse_share,
                )
                print("-------------------")
    ...
    def loss(self, k, total_batch=None):
        recon_loss, weighted_recon_loss, kl_loss, hsic_loss = self.model(
            self.a, self.b, k, **total_batch
        )
        ...
        loss = (
            weighted_recon_loss
            + self.calc_alpha_kl_coeff() * kl_loss
            + weighted_hsic
        )
        original_loss = (
            recon_loss
        ...
        self.iter_logs["unweighted_loss"].append(
            (recon_loss + kl_loss + hsic_loss).item()
        )
    ...
    def init_weights_from_pretrained(self, pretrained_state_dict):
        ...
        ), "Pre-trained state dictionary is required for initialization."
        if self.compatibility_check(pretrained_state_dict):
            ...
            for name, param in pretrained_state_dict.items():
                ...
                if name in model_state_dict:
                    if "decoder.L0.expr_L.weight" in name:
                        ...
                        original_mask = model_state_dict[
                            "decoder.L0.expr_L.mask"
                        ]
            ...
            print(
                "Successfully initialized the model with pre-trained weights."
            )
    ...
    def compatibility_check(self, pretrained_state_dict):
        ...
        for name, param in pretrained_state_dict.items():
            if name not in model_state_dict:
                raise KeyError(
                    f"{name} is not a valid parameter name in the current model."
                )
            if model_state_dict[name].shape != param.shape:
                raise ValueError(
                    f"Shape mismatch for layer {name}, model expected {model_state_dict[name].shape}, but got {param.shape} from pretrained weights."
                )
    ...
    def init_weights_pca(self):
        ...
        if not self.model.soft_mask:
            masked_init_weights = (
                pca.components_ * self.model.mask.cpu().numpy()
            )
        else:
            masked_init_weights = pca.components_

        self.model.decoder.L0.expr_L.weight.data.copy_(
            torch.tensor(masked_init_weights.T, dtype=torch.float32)
        )
    ...
    def init_weights_masked_pca(self):
        ...
        for i in range(self.model.latent_dim):
            ...
            masked_init_weights[i, self.model.mask[i, :] == 1] = (
                pca.components_
            )
