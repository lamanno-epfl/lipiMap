...
# Code: https://github.com/theislab/scarches/tree/master/scarches/models/trvae/modules.py
...
class Encoder(nn.Module):
    ...
    It will transform primary space input to means and log. variances of latent
    space with n_dimensions = z_dimension.
        ...
        if len(layer_sizes) > 1:
            ...
            for i, (in_size, out_size) in enumerate(
                ...
                if i == 0:
                    ...
                    print(
                        "\tHidden Layer", i, "in/out:", in_size, out_size
                    )
                    self.FC.add_module(
                        name="L{:d}".format(i),
...
class Decoder(nn.Module):
    ...
    It will transform the constructed latent space to the previous space of data with
    n_dimensions = x_dimension.
    ...
    dr_rate: Float
         Dropput rate applied to all layers, if `dr_rate`==0 no dropput will be applied.
    num_classes: Integer
         Number of classes (conditions) the data contain. if `None` the model will be
         a normal VAE instead of conditional VAE.
    """
    ...
    def __init__(
        ...
        if use_bn:
            ...
            self.FirstL.add_module(
                "N0",
                module=nn.LayerNorm(
                    layer_sizes[1], elementwise_affine=False
                ),
            )
