...
class BaseMixin:
    ...
    def _get_user_attributes(self):
        ...
        ]  # vars(self).items()]# if not name.startswith("__") and not name.startswith("_abc_")]
        attributes = [
    ...
    def _get_training_params(self):
        training_params = [
            (name, value) for name, value in self.trainer.__dict__.items()
        ]
        training_params = [a for a in training_params]
        training_params = {
            a[0]: a[1] for a in training_params if a[0] in training_params_to_keep
        }
    ...
    def create_model_directory(
        ...
        train_path = (
            f"{dataset}_{round(percentage*100)}percent_epochs{n_epochs}_"
    ...
    @classmethod
    def load(cls, dir_path: str, adata: Optional[AnnData] = None, map_location=None):
        ...
        attr_dict, train_dict, model_state_dict, var_names = cls._load_params(
            dir_path, map_location
        )
...
class SurgeryMixin:
    ...
    def load_query_data(
        ...
        """Transfer Learning function for new data. Uses old trained model and expands
        it for new conditions.
        ...
        freeze: Boolean
             If 'True' freezes every part of the network except the first layers of
             encoder/decoder.
...
class CVAELatentsMixin:
    def get_latent(
        ...
        ----------
        x
             Numpy nd-array to be mapped to latent space. `x` has to be in shape
             [n_obs, input_dim]. If None, then `self.adata.X` is used.
        c
        ...
        for batch in subsampled_indices:
            ...
            latent = self.model.get_latent(
                x_batch, c[batch] if c is not None else None, mean, mean_var
            )
...
class CVAELatentsModelMixin:
    def sampling(self, mu, log_var):
        """Samples from standard Normal distribution and applies re-parametrization
        trick. It is actually sampling from latent space distributions with N(mu, var),
        computed by encoder.
    ...
    def get_latent(self, x, c=None, mean=False, mean_var=False):
        ...
        ----------
        x:  torch.Tensor
             Torch Tensor to be mapped to latent space. `x` has to be in shape
             [n_obs, input_dim].
        c: torch.Tensor
    ...
    def get_y(self, x, c=None):
        """Map `x` in to the y dimension (First Layer of Decoder). This
        function will feed data in encoder  and return y for each sample in data.
