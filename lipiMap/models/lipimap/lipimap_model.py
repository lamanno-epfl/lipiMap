...
class LIPIMAP(BaseMixin, SurgeryMixin, CVAELatentsMixin):
    """Model for lipiMap class. This class contains the implementation of Conditional Variational Auto-Encoder.
       ...
       conditions: List
            ...
            For example, if condition_key refers to a categorical column, 'conditions' is a list of all categories.
       hidden_layer_sizes: List
            A list of hidden layer sizes for encoder network. Decoder network will be linear (one layer).
       latent_dim: Integer
    ...
    #         If True and `decoder_last_layer`='softmax', libary size encoder is used.
       ...
       soft_ext_mask: Boolean
            ...
            Used for query mapping.
       TODO: add more parameters to the docstring (e.g. logs, lipids_format, clipping_decoder_weights) 
    """
    ...
    def __init__(
        ...
        # representation_score_correction: bool = False,
    ):
        self.adata = adata
        
        if mask_key == 'dense':
            assert latent_dim is not None, 'Please provide latent_dim for a dense linear decoder.'
            print('The mask will automatically be made of ones for a dense linear decoder.')
            mask = np.ones((latent_dim, adata.n_vars)) # input_dim x latent_dim
        ...
        if condition_key == 'arbitrary':
            assert conditions is not None and conditions != [], 'Please provide conditions for arbitrary condition_key.'
        ...
        assert use_bn != use_ln, "Exactly one of use_bn or use_ln must be True"
        self.use_bn_ = use_bn
        self.use_ln_ = use_ln
        ...
        self.mask_ = mask if isinstance(mask, list) else mask.tolist()      # self.mask_ is a list
        ...
        # self.representation_score_correction_ = representation_score_correction
        # self.rho_ = self.adata.uns["rho"] if self.representation_score_correction_ else None
        ...
        self.model = lipiMap(
            ...
            # self.representation_score_correction_,
            # self.rho_,
        )
    ...
    def train(
        self,
        q, k,                                   # TODO: add documentation about weighted mse loss
        ...
        alpha_gl: Optional[float] = None,       # Group Lasso regularization coefficient
        omega: Optional[torch.Tensor] = None,   # Vector of coefficients for each group
        save_logs: bool = False,                # Save logs of the training process
        initialization: Optional[str] = None,   # Initialization method for the decoder weights
        **kwargs
    ):
        """Train the model.
    ...
    def term_lipids(self, term: Union[str, int], terms: Union[str, list]='terms'):
        """Return the dataframe with lipids belonging to the term after training sorted by absolute weights in the decoder.
        """
        if isinstance(terms, str):
            terms = list(self.adata.uns[terms]) # terms = 'lipidontology' for example... it is the mask_key I think
        else:
            terms = list(terms)
    ...
    def _get_init_params_from_dict(cls, dct):
        ...
        init_params = {
            ...
            'lipids_format': dct['lipids_format_'] if 'lipids_format_' in dct else 'norm_exp',
            'clipping_decoder_weights': dct['clipping_decoder_weights_'] if 'clipping_decoder_weights_' in dct else False,
            #'representation_score_correction': dct['representation_score_correction_'] if 'representation_score_correction_' in dct else False,
    ...
    def _validate_adata(cls, adata, dct):
        """
        Validates the AnnData object against the model's expected input dimensions and conditions.
