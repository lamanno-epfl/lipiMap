...
class lipiMap(nn.Module, CVAELatentsModelMixin):
    """lipiMap model class. This class contains the implementation of 
    ...
    mask: Tensor or None
         if not None, Tensor of 0s and 1s from utils.add_annotations to create 
         VAE with a masked linear decoder.
    decoder_last_layer: String or None
         The last layer of the decoder. Must be 'identity' (default for 'mse' loss),
    ...
    def __init__(
        ...
        recon_loss: str = "mse",  # Reconstruction loss: 'mse'
        ...
        decoder_last_layer: Optional[str] = None,  # 'identity' for mse-loss
        ...
        ext_mask: Optional[
            torch.Tensor
        ] = None,  # Lipid-LP matrix for the masked linear decoder for extension nodes
        ...
        if condition_key == "arbitrary":
            assert (
                len(conditions) > 0
            ), "If condition_key is 'arbitrary', conditions must be provided."
    ...
    def forward(self, a, b, k, x=None, batch=None, sizefactor=None, labeled=None):
        ...
        weighted_recon_loss = (
            (mse(recon_x, x) * (1 + k * weighting_mask)).sum(dim=-1).mean()
        )
        ...
        kl_div = (
            kl_divergence(
                ...
                Normal(
                    torch.zeros_like(z1_mean), torch.ones_like(z1_var)
                ),  # prior N(0,1)
            )
            .sum(dim=1)
            .mean()
